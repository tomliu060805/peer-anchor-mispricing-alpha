...
本项目的联动网络建在**残差**上——剔掉市场 beta 与行业均值后的那一半。
被扔掉的系统性那一半并非无用，只是服务于相反的目的：
特质成分用于定位错价（本项目），系统性成分用于分散化。

[`docs/systematic_half.md`](docs/systematic_half.md) 记录了对一个"把图建在因子暴露空间、
并让因子与图联合估计"的分散化框架的完整复现（`src/research/10~12`）。
**这条线不接生产策略**，两者目标相反，结论互不影响。

**结论：判负。** 四条判负项与依据：

| 判负项 | 依据 |
|---|---|
| 簇内度数倒数加权 | 低于**随机图零基准 2.1σ**（置换节点标签、度数分布保留，真实网络位置劣于随机指派） |
| "暴露图行业一致性更高" | 等稀疏度对照后**方向反转**（可用密度上 0.66–0.72×；相关图仅靠阈值截断即可超过原工作报告值） |
| 谱切分框架整体 | 三个协方差变体全部输给等权（≤0.125 vs 0.181） |
| 聚类稳定性作为卖点 | 联合估计 NMI 0.370，反不如样本协方差 0.719；月换手 47% |

可复现的是三项次级结论（协方差排序方向、条件数改善 26×、市场状态依赖）。
同一数据上，不需要预期收益的**最小方差（多头）以 6.82%／夏普 0.599／MDD 23.4% 胜过全部变体**，
月换手仅 10.7%——复杂框架要证明自己，得先赢过最小方差。
