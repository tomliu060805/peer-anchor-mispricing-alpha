# -*- coding: utf-8 -*-
"""v26b: v21完整配置下 N 扫描 {100,150,200,250,300,400}."""
...
def run_n(nh):
    ...
    for si,t in enumerate(sig5):
        ...
        selq&=np.nan_to_num(g['close'][t],nan=0)>=2.0
        nlv,_,_=feat5(t)
        ...
        for i2 in list(new_h):
            if ((rank[i2]>=3.0*nh) or np.isnan(s[i2])) and can_sell[i2]: del new_h[i2]
        for i2 in order:
            if len(new_h)>=nh: break
            if i2 in new_h or np.isnan(s[i2]) or not selq[i2]: continue
            new_h[i2]=0.0
        if len(new_h)<max(nh//4,30): holdings={}; continue
...
def ev(recs,name):
    ...
    return r
res={}
for nh in [100,150,200,250,300,400]:
    res[f'N{nh}']=ev(run_n(nh),f'N{nh}')
json.dump(res,open(f'{OUT}/metrics_v26b_nscan21.json','w'),ensure_ascii=False,indent=1)
print('done')
