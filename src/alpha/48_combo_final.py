...
close_raw=g['close']
NHOLD,BM=200,3.0
def run_c(nl5=False,lowpx=False):
    holdings={}; recs=[]
    for si,t in enumerate(sig5):
        ...
        if lowpx: selq&=np.nan_to_num(close_raw[t],nan=0)>=2.0
        if nl5:
            nlv,_,_=feat5(t)
...
def ev(recs,name):
    ...
    return r
res={}
res['base_v20']=ev(run_c(),'base_v20')
res['nl5_only']=ev(run_c(nl5=True),'nl5_only')
res['lowpx_only']=ev(run_c(lowpx=True),'lowpx_only')
res['both']=ev(run_c(nl5=True,lowpx=True),'both')
json.dump(res,open(f'{OUT}/metrics_v25b_final.json','w'),ensure_ascii=False,indent=1)
print('done')
