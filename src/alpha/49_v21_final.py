...
exec(head)
CH=PROJ+'/charts/v21_final_nav'
os.makedirs(CH,exist_ok=True)
recs=run_c(nl5=True,lowpx=True)
...
ds,pr,bq=ds[m],pr[m],bq[m]
ex=(1+pr)/(1+bq)-1
np.savez(f'{OUT}/port_v21_final.npz',dates=ds.astype('datetime64[D]').astype(str),port=pr,bench=bq,ex=ex)
...
stats={'ExAnn':round(float(nav[-1]**(ann/len(ex))-1),4),'IR':round(float(ex.mean()/ex.std()*np.sqrt(ann)),2),
 ...
 'avg_bp':round(float(ex.mean()*1e4),1),'pf':round(float(ex[ex>0].sum()/-ex[ex<0].sum()),2),'n':int(len(ex))}
...
plt.tight_layout(); plt.savefig(f'{CH}/v21_excess_vs_csiall.png',dpi=130); print('chart saved')
